...
func TestService_Login(t *testing.T) {
	...
	svc := auth.NewService(repo, users.NewService(repo), manager)
...
func TestService_Register(t *testing.T) {
	...
	t.Run("registrasi valid menerbitkan token untuk user baru", func(t *testing.T) {
		u, token, err := svc.Register(context.Background(), "Ani", "Ani", "ani@lattepos.com", "secret123")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if u.Username != "ani" {
	...
	t.Run("username terpakai dan input kosong dipetakan ke error domain", func(t *testing.T) {
		_, _, errTaken := svc.Register(context.Background(), "BUDI", "Budi", "budi@lattepos.com", "secret123")
		if !errors.Is(errTaken, users.ErrUsernameTaken) {
			t.Fatalf("expected ErrUsernameTaken, got %v", errTaken)
		}
		_, _, errEmpty := svc.Register(context.Background(), "", "", "", "")
