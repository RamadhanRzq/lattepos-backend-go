...
// RegisterRoutes mendaftarkan endpoint category yang ter-scope store+organisasi.
...
func RegisterRoutes(
	...
	mux.HandleFunc("POST /api/v1/org/{slug}/stores/{storeId}/categories",
		middleware.RequireOrgMember(verifier, orgs, h.Create))
	mux.HandleFunc("GET /api/v1/org/{slug}/stores/{storeId}/categories",
		middleware.RequireOrgMember(verifier, orgs, h.List))
	mux.HandleFunc("GET /api/v1/org/{slug}/stores/{storeId}/categories/{id}",
		middleware.RequireOrgMember(verifier, orgs, h.ByID))
	mux.HandleFunc("PUT /api/v1/org/{slug}/stores/{storeId}/categories/{id}",
		middleware.RequireOrgMember(verifier, orgs, h.Update))
	mux.HandleFunc("DELETE /api/v1/org/{slug}/stores/{storeId}/categories/{id}",
